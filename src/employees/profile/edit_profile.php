...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (!empty($new_password) || !empty($confirm_password)) {
        ...
        $sql = "UPDATE empleados 
                ...
                WHERE id_empleado = ?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("ssssssi", $nombre, $apellido_paterno, $apellido_materno, $telefono, $email, $hashed_password, $id_empleado);
    } else {
        $sql = "UPDATE empleados 
                SET nombre = ?, apellido_paterno = ?, apellido_materno = ?, telefono = ?, email = ?
                WHERE id_empleado = ?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("sssssi", $nombre, $apellido_paterno, $apellido_materno, $telefono, $email, $id_empleado);
    }
